...
void AP_InertialSensor_ExternalAHRS::handle_external(const AP_ExternalAHRS::ins_data_message_t &pkt)
{
    ...
    const enum Rotation rotation = (enum Rotation)_imu.get_external_ahrs_orientation();
...
void AP_InertialSensor_ExternalAHRS::start()
{
    ...
    if (_imu.register_gyro(gyro_instance, rate,
        ...
        set_external_ahrs_gyro_instance_number(gyro_instance);
        set_external_ahrs_accel_instance_number(accel_instance);
        started = true;
    }
}
