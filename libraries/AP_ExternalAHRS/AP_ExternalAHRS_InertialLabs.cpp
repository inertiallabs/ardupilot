...
bool AP_ExternalAHRS_InertialLabs::check_uart()
{
    ...
    if (GOT_MSG(BARO_DATA) &&
        ...
        AP::logger().WriteStreaming("ILB3", "TimeUS,IMS,Press,Diff,Temp,Alt,TAS,VWN,VWE,ArspSF,ADU,NewAD,ExtS",
                                    "s-PPOmnnnn--n",
                                    "F------------",
                                    "QIffffffffHHf",
                                    now_us, nav_ins_data.ms_tow,
    ...
    if (GOT_MSG(POSITION) &&
        ...
        AP::logger().WriteStreaming("ILB7", "TimeUS,IMS,Roll,Pitch,Yaw,VN,VE,VD,Lat,Lng,Alt,USW,USW2,Vdc,ISS",
                                    "s-dddnnnDUm--v-",
                                    "F--------------",
                                    "QIfffffffffHHfB",
                                    now_us, nav_ins_data.ms_tow,
    ...
    if (state2.unit_status != last_ins_status.unit_status) {
        send_EAHRS_status_report(last_ins_status.unit_status, state2.unit_status, IL_usw_msg, IL_usw_msg_size, IL_usw_last_msg_ms); // IL INS Unit Status Word (USW) messages

        if (state2.unit_status & IL_USW::MAG_VG3D_CLB_RUNTIME) {
        ...
        if (state2.unit_status & IL_USW::MAG_VG3D_CLB_SUCCESS) {
    ...
    if (state2.unit_status2 != last_ins_status.unit_status2) {
        send_EAHRS_status_report(last_ins_status.unit_status2, state2.unit_status2, IL_usw2_msg, IL_usw2_msg_size, IL_usw2_last_msg_ms); // IL INS Unit Status Word 2 (USW2) messages
    }

    if (state2.air_data_status != last_ins_status.air_data_status) {
        send_EAHRS_status_report(last_ins_status.air_data_status, state2.air_data_status, IL_adu_msg, IL_adu_msg_size, IL_adu_last_msg_ms); // IL Air Data Unit (ADU) messages
    }
...
void AP_ExternalAHRS_InertialLabs::get_filter_status(nav_filter_status &status) const
{
    ...
    status.flags.attitude = (now - last_att_ms < dt_limit) && (state2.ins_sol_status != 8); // 0

    status.flags.horiz_vel = (now - last_vel_ms < dt_limit) && (state2.ins_sol_status != 8); // 1
    status.flags.vert_vel = status.flags.horiz_vel; // 2

    status.flags.horiz_pos_abs = (now - last_pos_ms < dt_limit) && (state2.ins_sol_status != 8); // 3
    status.flags.horiz_pos_rel = status.flags.horiz_pos_abs; // 4
    status.flags.vert_pos = status.flags.horiz_pos_abs; // 5
    status.flags.pred_horiz_pos_rel = status.flags.horiz_pos_abs; // 8
    status.flags.pred_horiz_pos_abs = status.flags.horiz_pos_abs; // 9

    status.flags.using_gps = true; // 13
    status.flags.gps_quality_good = true; // 15

    status.flags.initalized = (state2.unit_status & IL_USW::INITIAL_ALIGNMENT_FAIL) == 0; // 16

    status.flags.rejecting_airspeed = false; // 17
}
...
void AP_ExternalAHRS_InertialLabs::send_status_report(GCS_MAVLINK &link) const
{
    ...
    // Send status flags
    const mavlink_eahrs_status_info_t package{state2.unit_status,
                                              state2.unit_status2,
