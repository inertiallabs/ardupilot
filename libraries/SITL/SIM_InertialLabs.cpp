...
void InertialLabs::send_packet(void)
{
    ...
    pkt.baro_data.baro_alt = fdm.altitude * 100;
    ...
    pkt.temperature = KELVIN_TO_C(SSL_AIR_TEMPERATURE * theta) * 10;
    ...
    if (packets_sent % gps_frequency == 0) {
        ...
        // 0x30 GNSS Position
        pkt.gnss_position.gnss_lat = fdm.latitude * 1e7;  // deg*1.0e7
        pkt.gnss_position.gnss_lon = fdm.longitude * 1e7; // deg*1.0e7
        pkt.gnss_position.gnss_alt = fdm.altitude * 1e2;  // m*100
        ...
        pkt.gnss_angles.gnss_heading = 0; // only for dual-antenna GNSS receiver
        pkt.gnss_angles.gnss_pitch = 0; // only for dual-antenna GNSS receiver
